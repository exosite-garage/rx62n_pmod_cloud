...
static u16_t ledvalue = 0;
static u16_t Pmodvalue = 0;
static char ledstate = 1;
static u16_t ping_value = 31;
char alias_str[20];
...
#define DA_CONVERTER    "daout2"
#define D_POTENTIOMETER "dpout"
#define PING            "ping"
//PMOD: AD4, DA3, DPOT
//#define USE_PMODAD4
//#define USE_PMODDA3
...
#ifdef USE_PMODAD4	    
		volts = (int)Exosite_AD7980_Conversion(); 	
        sprintf(alias_str,"%s=%d.%d",ADC_SEND_ALIAS,volts/100,volts%100);
#else
        volts = (usADC_Result*33/1024);
	    sprintf(alias_str,"%s=%d.%d&%s=%d",ADC_SEND_ALIAS,volts/10,volts%10,PING,ping_value);
...
#ifdef USE_PMODDA3		
        sprintf(alias_str,"%s&%s",LED_CTRL_ALIAS,DA_CONVERTER);
#else	
#ifdef  USE_PMODPOT
        sprintf(alias_str,"%s&%s",LED_CTRL_ALIAS,D_POTENTIOMETER);
#else
...
#ifdef USE_PMODPOT
      if (!Exosite_Get(alias_str,D_POTENTIOMETER)) break;
      write_to_lcd(4,alias_str); 
      {
        char len,i,j = 0;
        unsigned short value = 0;    
        ...
        //convert up to four digits into an integer
        for (i=0; i<len; i++){
          if (0 != j) value = value*10+atoi(alias_str[i]);  
            else value += atoi(alias_str[i]);
          j--;
        }
		Pmodvalue = value;
		//Write the value to PMODPOT
		//Write the value to PMODPOT
		Exosite_AD5160_Set(Pmodvalue % 255);
	  } 
#endif
	  
#ifdef USE_PMODDA3
      if (!Exosite_Get(alias_str,DA_CONVERTER)) break;
      write_to_lcd(4,alias_str); 
      {
        char len,i,j = 0;
        unsigned short value = 0;    
